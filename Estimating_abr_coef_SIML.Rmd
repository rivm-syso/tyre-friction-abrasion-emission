...
FW_plot <- ggplot(kData25, 
                  aes(x = c(AbrasionTest), y = FW_p_km, fill=reorder(AbrasionTest, FW_p_km))) +
  geom_violin()+
...
ACplot1 <- ggplot(kData25, 
                  aes(x = c(AbrasionTest), y = AbrasionCoeff,fill = AbrasionTest )) +
  geom_violin() +theme(legend.position="none")+
  # geom_point(aes(colour = reorder(TestScenName, AbrasionCoeff)), 
             # position = position_jitterdodge(jitter.width = 0.05,
                                             # dodge.width = 0.75))+
  labs(x = "Abrasion test scenario", y = "Abrasion Coeff (mg/J)") +                   # Adjust labels
