...
f_c_accel_max <- function(t_0_100kmh) {
  (100 * (1000 / 3600)) / t_0_100kmh
}
...
if (track_underground == "dry_asphalt") {
  optimal_slip = runif(n = 1000, 0.15, 0.20)
}
if (track_underground == "wet_asphalt") {
  optimal_slip = runif(n = 1000, 0.08, 0.12)
}
...
f_mu_max_wet <-
  function(grip_index_tyre, mu_max_ref_tyre_wet) {
    (grip_index_tyre * mu_max_ref_tyre_wet) / 1.25
  }
f_mu_max <-
  function(grip_index_tyre,
           x_correct_road,
           mu_max_ref_tyre_wet) {
    f_mu_max_wet(grip_index_tyre, mu_max_ref_tyre_wet) * x_correct_road
  }
...
f_c_full_brake_wet <-
  function (grip_index_tyre,
            c_full_brake_ref_tyre_wet) {
    (grip_index_tyre * c_full_brake_ref_tyre_wet) / 1.25
  }
...
f_normal_load_force <-
  function(m_vehicle, grav_constant) {
    m_vehicle * grav_constant
  }
...
## sector acceleration
f_accel_time <-
  function(sector_start_velocity_kmh,
           sector_velocity_kmh,
           c_accel) {
    -(sector_start_velocity_kmh * 1000 / 3600 - sector_velocity_kmh * 1000 /
        3600) / c_accel
  }
f_accel_distance <-
  function(sector_start_velocity_kmh,
           sector_velocity_kmh ,
           c_accel) {
    sector_start_velocity_kmh * 1000 / 3600 * f_accel_time(sector_start_velocity_kmh, sector_velocity_kmh, c_accel) +
      1 / 2 * c_accel * (f_accel_time(sector_start_velocity_kmh, sector_velocity_kmh, c_accel)) ^
      2
  }
f_accel_v_start <-
  function(sector_velocity_kmh,
           sector_start_velocity_kmh) {
    pmin(sector_velocity_kmh, sector_start_velocity_kmh) * 1000 / 3600
  }
f_accel_average_v <-
  function(sector_start_velocity_kmh,
           sector_velocity_kmh) {
    (
      f_accel_v_start(sector_velocity_kmh, sector_start_velocity_kmh) + sector_velocity_kmh *
        1000 / 3600
    ) / 2
  }
f_accel_drag_force <-
  function (c_drag,
            ...
            sector_velocity_kmh,
            v_wind) {
    pmax(0, c_drag * A_vehicle * rho_air * (((
      f_accel_average_v(sector_start_velocity_kmh, sector_velocity_kmh)
    ) + v_wind) ^ 2) / 2)
  }
f_accel_roll_force <- # This function is exaclty the same and repeated for acc, decel and const... See base functions f_roll_force
  function(c_roll, m_vehicle, grav_constant) {
...
f_accel_long_force <-
            ...
            c_accel)
  {
    f_accel_drag_force(
      ...
      sector_velocity_kmh,
      v_wind
    ) + f_accel_roll_force(c_roll, m_vehicle, grav_constant)
    + f_accel_uphill_slope_force(m_vehicle, grav_constant, alpha_slope) +
...
f_accel_friction_work <-
  function(optimal_slip,
           x_slip_long_force,
           c_drag,
           A_vehicle,
           rho_air,
           ...
           c_accel)
  {
    f_accel_distance(sector_start_velocity_kmh, sector_velocity_kmh , c_accel) *
      f_accel_long_force(
        c_drag,
        A_vehicle,
        rho_air,
...
f_decel_time <-
  function(sector_velocity_kmh,
           sector_end_velocity_kmh,
           c_decel) {
    ((sector_velocity_kmh * 1000 / 3600) - (sector_end_velocity_kmh * 1000 /
                                              3600)) / c_decel
  }
f_decel_distance <-
  function(sector_velocity_kmh,
           sector_end_velocity_kmh ,
           c_decel) {
    ((sector_velocity_kmh * 1000 / 3600) ^ 2 - (sector_end_velocity_kmh * 1000 /
                                                  3600) ^ 2) / (2 * c_decel)
  }
f_decel_v_start <-
  function(sector_velocity_kmh,
           sector_end_velocity_kmh) {
    pmax(sector_velocity_kmh, sector_end_velocity_kmh) * 1000 / 3600
  }
f_decel_average_v <-
  function (sector_velocity_kmh,
            sector_end_velocity_kmh) {
    (
      f_decel_v_start(sector_velocity_kmh, sector_end_velocity_kmh) + sector_end_velocity_kmh *
        1000 / 3600
    ) / 2
  }
f_decel_drag_force <-
  function (c_drag,
            ...
            v_wind) {
    pmax(0, c_drag * A_vehicle * rho_air * (((
      f_decel_average_v(sector_velocity_kmh, sector_end_velocity_kmh)
    ) + v_wind) ^ 2) / 2)
  }
f_decel_roll_force <-
  function(c_roll, m_vehicle, grav_constant) {
...
f_decel_resist_force <-
  function (c_drag,
            ...
            alpha_slope)
  {
    f_decel_drag_force(c_drag,
                       A_vehicle,
                       rho_air,
                       sector_velocity_kmh,
                       sector_end_velocity_kmh ,
                       v_wind) + f_decel_roll_force(c_roll, m_vehicle, grav_constant) + f_decel_uphill_slope_force(m_vehicle, grav_constant, alpha_slope)
  }
f_c_decel_resist <-
  function (c_drag,
            A_vehicle,
...
f_c_accel_downhill_slope_force <-
  function(m_vehicle,
           ...
           m_rotate) {
    -(
      f_downhill_slope_force(m_vehicle, grav_constant, alpha_slope) / (m_vehicle +
                                                                         m_rotate)
    )
  }
f_c_add_brake <-
...
f_decel_friction_work <-
  function(optimal_slip,
           ...
           c_full_brake_ref_tyre_wet)
  {
    f_decel_distance(sector_velocity_kmh, sector_end_velocity_kmh , c_decel) *
      f_decel_long_force(
        c_drag,
        A_vehicle,
        rho_air,
...
f_const_speed_distance <-
  function (sector_distance,
            sector_start_velocity_kmh,
            sector_velocity_kmh ,
            c_accel,
            sector_end_velocity_kmh ,
...
f_const_speed_drag_force <-
  function (c_drag,
            ...
            v_wind)
  {
    pmax(0, c_drag * A_vehicle * rho_air * (((sector_velocity_kmh * 1000 / 3600) +
                                               v_wind
    ) ^ 2) / 2)
  }
f_const_speed_roll_force <-
  function(c_roll, m_vehicle, grav_constant) {
    c_roll * m_vehicle * grav_constant
  }
f_const_speed_uphill_slope_force <-
  function(m_vehicle, grav_constant, alpha_slope) {
    pmax(0, m_vehicle * grav_constant * sin(alpha_slope))
  }

f_const_speed_downhill_slope_force <-
  function (m_vehicle, grav_constant, alpha_slope) {
    pmin(0, m_vehicle * grav_constant * sin(alpha_slope))
  }
f_const_speed_add_brake_force <-
  function(m_vehicle,
           grav_constant,
           alpha_slope,
           c_drag,
           ...
           c_roll)
  {
    pmax(
      0,
      -f_const_speed_downhill_slope_force(m_vehicle, grav_constant, alpha_slope) -
        f_const_speed_drag_force(c_drag, A_vehicle, rho_air, sector_velocity_kmh, v_wind)
      - f_const_speed_roll_force(c_roll, m_vehicle, grav_constant)
    )
  }

f_const_speed_resist_force <-
  function(c_drag,
           A_vehicle,
           rho_air,
           sector_velocity_kmh,
  ...
  {
    f_const_speed_drag_force(c_drag, A_vehicle, rho_air, sector_velocity_kmh, v_wind) +
      f_const_speed_roll_force(c_roll, m_vehicle, grav_constant) + f_const_speed_uphill_slope_force(m_vehicle, grav_constant, alpha_slope)
  }
    ...
    pmin(
      ...
      (
        f_const_speed_resist_force(
          ...
          grav_constant,
          alpha_slope
        ) / f_normal_load_force(m_vehicle, grav_constant) / x_slip_long_force
      )
    )
  }

f_const_speed_c_add_brake <-
  function(m_vehicle,
           grav_constant,
           alpha_slope,
           c_drag,
           ...
           c_full_brake_ref_tyre_wet)
  {
    pmin(
      ...
      f_const_speed_c_add_brake(
        ...
        c_roll,
        m_rotate
      ) / f_c_full_brake(grip_index_tyre, x_correct_road, c_full_brake_ref_tyre_wet)
    )
  }
...
f_sector_longitude_friction_work <-
  function(sector_distance,
           sector_start_velocity_kmh,
           sector_velocity_kmh ,
           c_accel,
...
f_corner_distance <-
  function(sector_corner_radius,
           sector_corner_angle) {
    (sector_corner_angle / (360) * 2 * sector_corner_radius * pi)
  }
f_corner_centripet_force <-
  function(m_vehicle ,
           sector_velocity_kmh,
           sector_corner_radius) {
    m_vehicle * ((sector_velocity_kmh * 1000 / 3600) ^ 2) / sector_corner_radius
  }
f_corner_bank_force <-
...
f_corner_lat_friction_work <-
  function(sector_corner_radius,
           ...
           optimal_slip)
  {
    f_corner_distance(sector_corner_radius, sector_corner_angle) * f_corner_lat_force(
      m_vehicle ,
      sector_velocity_kmh,
      sector_corner_radius ,
      grav_constant,
...
f_corner_long_friction_work <-
  ...
  {
    f_sector_longitude_friction_work(
      f_corner_distance(sector_corner_radius, sector_corner_angle),
      sector_start_velocity_kmh,
      sector_velocity_kmh ,
      c_accel,
      sector_end_velocity_kmh ,
