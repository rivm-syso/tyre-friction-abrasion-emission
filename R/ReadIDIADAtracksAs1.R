#Calculate all to Watt / km
#step 1: one round to fractions 

#Basic forces
ownWindF <- function (c_drag,
          A_vehicle,
          rho_air,
          velocity_kmh
...
WearRLLong <- tidyr::pivot_longer(data = WearWheelsWideAgain, 
                                  ...
                                  names_to = "LRatio",
                                  values_to = "wear")
ggplot(data = WearRLLong, aes(x = LRatio, y = wear, fill = LRatio)) +
  geom_violin(trim = F) +
...
rf <- randomForest(wear~ Tyre + track + FrOrRear,WearAsLong)
varImpPlot(rf, main = "wear [mg/km] trend R123" )

#?partialPlot(rf, pred.data = WearAsLong, x.var = Wheel)
