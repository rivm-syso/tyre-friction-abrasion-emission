...
FWtotals <-
  ...
  summarise(sector_count = n(),
            PartFrictionWork = sum(FricWork_p_sector)) |> # sum up all friction work per test section (e.g. rural 60 kph)
...
plot <- ggplot(FWtotals, aes(x = c(AbrasionTest), y = FW_p_km, fill=AbrasionTest )) +
  geom_boxplot()
